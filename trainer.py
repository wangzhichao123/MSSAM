...
def trainer_synapse(args, model, snapshot_path, multimask_output, low_res):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            image_batch, label_batch, low_res_label_batch = image_batch.cuda(), label_batch.squeeze(1).cuda(), low_res_label_batch.squeeze(1).cuda()
            ...
            loss_ce = ce_loss(mask, low_res_label_batch[:].long())
            loss_dice = dice_loss(mask, low_res_label_batch, softmax=True)
            
            loss_ce_1 = ce_loss(diy_decoder_mask, low_res_label_batch[:].long())
            loss_dice_1 = dice_loss(diy_decoder_mask, low_res_label_batch, softmax=True)
            loss = loss + (lc1 * loss_ce + (1 - lc1) * loss_dice) + (lc1 * loss_ce_1 + (1 - lc1) * loss_dice_1)
